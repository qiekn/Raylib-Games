...
inline void Rect2Points(double x, double y, double w, double h, Color col) {
    ...
    Rectangle rect = {static_cast<float>(x), static_cast<float>(y), static_cast<float>(w - x), static_cast<float>(h - y)};
...
inline void DrawCenteredRect(double x, double y, double w, double h, Color col) {
    w = std::abs(w);
    h = std::abs(h);
    DrawRectangleRec({static_cast<float>(x - w * 0.5), static_cast<float>(y - h * 0.5), static_cast<float>(w), static_cast<float>(h)}, col);
}
...
inline void DrawRotatedRect(double x1, double y1, double x2, double y2, double rad, Color col) {
    ...
    dist = sqrt(dist);
    Rectangle rec = {static_cast<float>(0.5 * (x1 + x2)), static_cast<float>(0.5 * (y1 + y2)), static_cast<float>(dist), static_cast<float>(2 * rad)};
