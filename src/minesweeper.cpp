...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    if (state == LOSE) {
      ...
      DrawText(youLose, screenWidth / 2 - MeasureText(youLose, 40) / 2,
               screenHeight / 2 - 10, 40, DARKGRAY);
      DrawText(pressRToRestart,
               screenWidth / 2 - MeasureText(pressRToRestart, 20) / 2,
               screenHeight * 0.75f - 10, 20, DARKGRAY);

      int minutes = (int)(timeGameEnded - timeGameStarted) / 60;
      int seconds = (int)(timeGameEnded - timeGameStarted) % 60;
      DrawText(
          TextFormat("Time played: %d minutes, %d seconds.", minutes, seconds),
          20, screenHeight - 40, 20, DARKGRAY);
    }

    if (state == WIN) {
      DrawRectangle(0, 0, screenWidth, screenHeight, Fade(WHITE, 0.8f));
      DrawText(youWin, screenWidth / 2 - MeasureText(youWin, 40) / 2,
               screenHeight / 2 - 10, 40, DARKGRAY);
      DrawText(pressRToRestart,
               screenWidth / 2 - MeasureText(pressRToRestart, 20) / 2,
               screenHeight * 0.75f - 10, 20, DARKGRAY);
    ...
    if (cell.containsMine) {
      ...
      if (cell.nearbyMines > 0) {
        DrawText(TextFormat("%d", cell.nearbyMines), cell.i * cellWidth + 12,
                 cell.j * cellHeight + 4, cellHeight - 8, DARKGRAY);
      }
    }
  } else if (cell.flagged) {
    // draw flag
    Rectangle source = {0, 0, static_cast<float>(flagSprite.width),
                        static_cast<float>(flagSprite.height)};
    Rectangle dest = {static_cast<float>(cell.i * cellWidth),
                      static_cast<float>(cell.j * cellHeight), cellWidth,
                      cellHeight};
    Vector2 origin = {0, 0};
